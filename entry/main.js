...
import { createProvider } from "../src/assets/lib/utils";

import App from "./App";

const { appName } = require("../package.json");
// 创建history模式
const { webHistory } = window.config;
const createWebHistoryFunc = {
...
async function render(props) {
  ...
  const { registerStore, registerComponent, registerRoute, registerHook } =
    await import("../src");
  ...
  if (registerHook) {
    ...
    const globalHookKyes = Object.getOwnPropertySymbols(globalHookMap);
    if (globalHookKyes.length) {
      app.use(createProvider(globalHookMap));
    }
  }
  // 挂载应用
